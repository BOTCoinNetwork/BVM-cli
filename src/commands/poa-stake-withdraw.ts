...
export default (evmlc: Vorpal, session: Session) => {
    const description = 'Withdraw staked BOC tokens';

    return evmlc
        .command('stake withdraw [value]')
        .alias('s w')
        ...
        .action(
            (args: Args): Promise<void> =>
                new StakeWithdrawCommand(session, args).run()
        );
};

class StakeWithdrawCommand extends Command<Args> {
    ...
    protected async check(): Promise<void> {
        if (!this.args.options.value) {
            throw Error('Withdrawal amount not provided');
        }
    ...
    protected async exec(): Promise<string> {
        ...
        if (!receipt.logs.length) {
            this.debug('Withdrawal failed - Gas or insufficient balance');
        }
...
export const StakeWithdraw = StakeWithdrawCommand;